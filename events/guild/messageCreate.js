...
client.on("messageCreate", async (message) => {
   ...
   if (command.toggleOff) {
      let toggleoff_embed = new MessageEmbed()
         ...
         .setFooter(`${clientname}`, `${clientavatar}`)
         .setTimestamp();
      return message.reply({ embeds: [toggleoff_embed] }).catch(console.error);
      ...
      let userperms_embed = new MessageEmbed()
         ...
         .setFooter(`${clientname}`, `${clientavatar}`)
         .setTimestamp();
      return message.reply({ embeds: userperms_embed }).catch(console.error);
      ...
      let botperms_embed = new MessageEmbed()
         ...
         .setFooter(`${clientname}`, `${clientavatar}`)
         .setTimestamp();
      return message.reply({ embeds: [botperms_embed] }).catch(console.error);
      ...
      if (!developerID.includes(message.author.id)) {
         let developersOnly_embed = new MessageEmbed()
            ...
            .setFooter(`${clientname}`, `${clientavatar}`)
            .setTimestamp();
         return message.reply({ embeds: [developersOnly_embed] }).catch(console.error);
      }
      ...
      let timecommand = prettyMilliseconds(command.timeout * 1000, { verbose: true, verbose :true })
        ...
        return message.channel.send({embeds: [cooldownEmbed]}).catch(console.error);
    } else {
